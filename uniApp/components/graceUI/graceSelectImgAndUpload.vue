    ...
    methods:{
            ...
            uni.chooseImage({
                ...
                success:(res) => {
					for(let i = 0; i < res.tempFilePaths.length; i++){
						let suffix = res.tempFiles[i]["name"] == undefined ? res.tempFiles[i]["path"].split(".")[1] : res.tempFiles[i]["name"].split(".")[1];
						let fileName = common.createGuid() + "."+suffix;
						this.imgLists.push({url : res.tempFilePaths[i], progress : 0, error : false,file:res.tempFiles[i]
						,fileName:fileName});
					}
                    uni.hideLoading();
                },
		...
		uploadBase : function (index) {
			...
			if(index > (this.imgLists.length - 1)){
				...
				this.$emit('uploaded', this.loadedList);
				return ;
			}
			// 验证后端
			...
			let url = this.uploadServerUrl + "?fileName=" +this.imgLists[index]["fileName"] ;
			// 创建上传对象
