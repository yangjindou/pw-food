<template>
	<view class="gui-search" :style="{height:height, backgroundColor:background,borderRadius:borderRadius}">
		...
		@input="inputting" @confirm="confirm" :focus="focus" 
		:placeholder-class="placeholderClass" />
		<view class="gui-search-input" v-if="disabled" @tap.stop="tapme" 
		:style="{height:inputHeight, lineHeight:inputHeight, fontSize:inputFontSize, color:iconColor}">{{placeholder}}</view>
...
.gui-search-input{width:100%; margin:0 10rpx; border:none; padding:0; background:rgba(255,255,255,0);}
</style>