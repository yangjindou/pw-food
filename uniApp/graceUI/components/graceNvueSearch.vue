<template>
	<view class="gui-search" :style="{height:height, backgroundColor:background,borderRadius:borderRadius}">
		...
		<input type="text" class="gui-search-input" :placeholder="placeholder" v-model="inputVal" :focus="focus" 
		:style="{height:inputHeight, lineHeight:inputHeight, fontSize:inputFontSize, color:inputColor}" 
		@input="inputting" @confirm="confirm" :placeholder-class="placeholderClass" v-if="!disabled" />
		<text class="gui-search-input" :style="{height:inputHeight, lineHeight:inputHeight, fontSize:inputFontSize, color:iconColor}" 
		@tap.stop="tapme" v-if="disabled">{{placeholder}}</text>
...
.gui-search-input{width:600rpx; margin:0 10rpx; flex:1; border-width:0; padding:0; background-color:rgba(255,255,255,0);}
</style>