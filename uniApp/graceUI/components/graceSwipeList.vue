<template>
	<scroll-view class="grace-swipe-list" :scroll-y="scY" 
	:style="{width:width+'rpx', height:height+'px'}">
			...
			:style="{width:width+'rpx'}">
				<view class="grace-swipe-list-item-body" :style="{
					...
					<view class="grace-swipe-list-img" 
					:style="{width:imgSize[0], height:imgSize[1]}" @tap="itemTap(index)">
						<image :src="item.img" mode="widthFix" 
						:style="{width:imgSize[0], height:imgSize[1], borderRadius:'6rpx'}"></image>
						<view class="grace-swipe-list-point" v-if="item.msgnumber > 0">{{item.msgnumber}}</view>
					</view>
					<view class="grace-swipe-list-content">
						<graceTouch @thStart="thStart" @thMove="thMove" @thEnd="thEnd" 
						:datas="[index]" @tapme="tapin">
...
export default{
	...
	methods:{
		...
		tapin : function () {
			if(this.oX < 30){
				this.scY = true;
				this.$emit('itemTap',this.moveIndex);
				this.moveIndex = -1;
			}
		}
	},
