...
//            return errorReturn(response, ResultEnums.E_PARAM_MISS);
//        }
//        try {
//            // 通过token解析获取userId，从redis中获取user对象，并且验证密码是否正确
//            String userId = JWT.decode(token).getAudience().get(0);
//            if (!redisUtils.hasKey(RedisUtils.USER_PREFIX + userId)) {
//                return errorReturn(response, ResultEnums.E_USER_FAILURE);
//            }
...
//        return true;
    ...
    private boolean errorReturn(HttpServletResponse response, ResultEnums hsCode) throws IOException {
        response.setCharacterEncoding("UTF-8");
        response.setContentType("text/html;charset=utf-8");
        PrintWriter writer = response.getWriter();
        writer.print(JSONObject.toJSON(new ResultEntity(hsCode)));
        writer.flush();
        return false;
    }
}
